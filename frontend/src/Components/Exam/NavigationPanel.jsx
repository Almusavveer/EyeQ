        ...
        <div className="bg-white p-6 rounded-xl shadow-lg h-full">
            ...
            <div className="grid grid-cols-5 md:grid-cols-4 lg:grid-cols-5 gap-3">
                {questions.map(q => {
                    ...
                    let statusClass = 'bg-gray-200 text-gray-700 hover:bg-gray-300 cursor-pointer'; // Not Answered
                    if (q.id === currentQuestionId) {
                        statusClass = 'bg-blue-500 text-white ring-2 ring-offset-2 ring-blue-500 cursor-pointer'; // Current
                    } else if (q.isFlagged) {
                        statusClass = 'bg-yellow-400 text-white hover:bg-yellow-500 cursor-pointer'; // Flagged
                    } else if (q.answer != null) {
                        statusClass = 'bg-green-500 text-white hover:bg-green-600 cursor-pointer'; // Answered
                    }
